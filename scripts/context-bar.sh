#!/bin/bash
# Refs: https://github.com/ykdojo/claude-code-tips/blob/main/scripts/README.md
...
# Build output: Model | Context | Prompt (all on one line)
output="${C_ACCENT}${model}${C_GRAY}"
# [[ -n "$dir" ]] && output+=" | 📁${dir}"
# [[ -n "$branch" ]] && output+=" | 🔀${branch} ${git_status}"
output+=" | ${ctx}"

# Add user's last message on same line
if [[ -n "$last_user_msg" ]]; then
    # Truncate if too long (max 60 chars for the message)
    max_msg_len=60
    if [[ ${#last_user_msg} -gt $max_msg_len ]]; then
        output+=" | 💬 ${last_user_msg:0:$((max_msg_len - 3))}..."
    else
